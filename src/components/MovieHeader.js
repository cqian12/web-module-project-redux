import React from 'react';
import { Link } from 'react-router-dom';
import { connect } from 'react-redux';
import { toggleFaves } from '../actions/favesActions';

const MovieHeader = (props) => {
    const appTitle = props.appTitle
    const displayFavorites = props.displayFavorites;

    const faveToggle = () => {
        props.toggleFaves()
    }
    
    return(<div className="table-title">
        <div className="row">
        <div className="col-sm-6">
            <h2>{appTitle}</h2>
        </div>
        <div className="col-sm-6 headerBar">
            <div className="btn btn-sm btn-primary" onClick={faveToggle}><span>{ displayFavorites ? "Hide" : "Show"} Favorites</span></div>
            <Link to="/movies" className="btn btn-sm btn-primary">View All Movies</Link>
            <Link to="/movies/add" className="btn btn-sm btn-success"><i className="material-icons">&#xE147;</i> <span>Add New Movie</span></Link>
        </div>
        </div>
    </div>);
}

const mapStateToProps = (state) => {
    return {
        appTitle: state.movieReducer.appTitle,
        displayFavorites: state.favesReducer.displayFavorites
    }
}

export default connect(mapStateToProps,{toggleFaves})(MovieHeader);